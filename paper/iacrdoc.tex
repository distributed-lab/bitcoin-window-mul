...
\end{center}
The paper is structured as follows: in \Cref{section:preliminaries} we will give a basic overview of Bitcoin Script and fast multiplication methods. In \Cref{section:implementation} we will list scripts to conduct the windowed multiplication (our primary proposed method). Finally, in \Cref{section:discussion} and \Cref{section:conclusion}, we will compare our performance with state-of-the-art and draw a conclusion.
...
Now, all we are left to do is perform the algorithm similar to \Cref{alg:script_addition_nooverflow}, but with small optimizations, accounting for the fact that we do not need \texttt{OP\_ZIP}. The implementation is specified in \Cref{alg:script_doubling_no_overflow}.
...
So the implementation of \Cref{alg:double_and_add} in \textit{Bitcoin Script} is specified in \Cref{alg:script_double_and_add}. As can be seen, the cost (in opcodes) of conducting the double-and-add algorithm is $N\mathsf{A}+(N-1)\mathsf{D}$. Note that when analyzing the cost in \Cref{section:multiplication_methods}, we specified the \textit{maximal} number of additions that get performed, but here the situation is different: the number of additions is exactly $N$, despite the fact that the \texttt{OP\_IF} branch might be executed only a few times.
...
\For{$i \in \{1,\dots,L-1\}$} {
    ...
    \script{ \opcode{BigInt<\text{$\lambda$}>::OP\_ADD} } \Comment*[r]{Since we need to only care about the last limbs, we do not extend the result}          
}
...
\begin{enumerate}
    ...
    \item \textit{BitVM $29\times 9$ Karatsuba Multiplication}: This is the most recent version that \textit{BitVM} mostly relies on that uses the Karatsuba multiplication (see \Cref{section:karatsuba}) with $(n=29,\ell=9)$ to represent a $254$-bit integer.
\end{enumerate}

The comparison results are depicted in \Cref{tab:mulcomparison}.
...
\Input{Points $P,Q \in E(\mathbb{F}_q)$ and scalars $\alpha,\beta \in \mathbb{Z}_r$}
\Output{Result of $[\alpha]P \oplus [\beta]Q \in E(\mathbb{F}_q)$}
...
\Return{Point $R \in E(\mathbb{F}_q)$}
\label{alg:double_multiplication}
\end{algorithm}

\begin{remark}
    Note that in \Cref{alg:double_multiplication} we can alternatively store the single lookup table, say $\mathcal{T}_{P,Q}[i,j]$, which has all possible precomputed values $[i]P \oplus [j]Q$ for $i,j \in \{0,\dots,2^{w}-1\}$. However, that would make the lookup table of size $2^{2w}$ (which, again, must be stored in the stack) compared to $2 \times 2^{w}$, making it impractical for Bitcoin Script.
\end{remark}
...
That being said, \Cref{alg:double_multiplication} can be used to optimize calculating $xy-pq$. Indeed, change $\alpha$ and $\beta$ to $x$ and $p$, while $P$ and $Q$ to $y$ and $q$, respectively, as we did in the previous sections. Moreover, since $p$ is fixed, it can be decomposed beforehand, without needing to do that in run-time. 
...
Consider another problem typical for elliptic curve arithmetic: suppose $P \in E(\mathbb{F}_q)$ is fixed, and we know it in compile-time. Then, we are given the scalar $\alpha \in \mathbb{Z}_r$ and asked to compute $[\alpha]P$. Since $P$ is fixed, we can precompute some data that depends solely on $P$ beforehand. For example, in \Cref{alg:windowed}, we can precompute expressions of a form $[2^{wi}]P$ for $i \in \{0,\dots,L-1\}$ and apply the \Cref{alg:fixed_point_mul}. 

\begin{algorithm}
\caption{$w$-width windowing method for fixed point multiplication}

\Input{Point $P \in E(\mathbb{F}_q)$ known in compile-time and scalar $\alpha \in \mathbb{Z}_r$}
\Output{Result of $[\alpha]P$}
...
\Return{Point $A \in E(\mathbb{F}_q)$}
